...
def classify_title(title: str) -> int:
    ...
    if title in NON_TECH_TITLES:
        return 1
    # Unknown titles: check for tech keywords (substring match)
    tl = title.lower()
    if any(t in tl for t in ["ml ", "machine learning", "ai ", "data scien", "nlp"]):
        return 4
    if any(t in tl for t in ["engineer", "developer", "scientist", "architect"]):
        return 2
    if "analyst" in tl:
        return 2
    return 1
...
def calculate_experience_score(yoe: float) -> float:
    ...
    Clean piecewise ramp with no discontinuities.
    """
    if IDEAL_YOE_MIN <= yoe <= IDEAL_YOE_MAX:
        return 1.0
    if ACCEPTABLE_YOE_MIN <= yoe < IDEAL_YOE_MIN:
        # 3-5 years: linear ramp 0.0 -> 0.7
        return 0.7 * (yoe - ACCEPTABLE_YOE_MIN) / (IDEAL_YOE_MIN - ACCEPTABLE_YOE_MIN)
    if IDEAL_YOE_MAX < yoe <= ACCEPTABLE_YOE_MAX:
        # 9-14 years: linear decay 0.8 -> 0.0
        return 0.8 * (1.0 - (yoe - IDEAL_YOE_MAX) / (ACCEPTABLE_YOE_MAX - IDEAL_YOE_MAX))
    return 0.0
...
def analyze_career_history(candidate: Candidate, features: CandidateFeatures) -> CandidateFeatures:
    ...
    if features.career_job_count > 0:
        ...
        # Career progression: reward steady tenure and title variety
        ...
        elif features.avg_tenure_months > 24:
            # Steady deep workers (same title, long tenure) also get some credit
            features.career_progression_score = min(0.5, features.avg_tenure_months / 60.0)
...
def build_text_for_embedding(candidate: Candidate, features: CandidateFeatures) -> CandidateFeatures:
    """Build combined candidate document for embedding (headline + summary + career)."""
    p = candidate.profile
